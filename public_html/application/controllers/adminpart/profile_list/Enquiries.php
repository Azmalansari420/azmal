...
class Enquiries extends Admin_Controller {
	...
	public function __construct(){
		...
		$this->breadcrumb(array("profile_list/enquiries"=>"Enquiries"));
	...
	public function index(){
		...
		$query = $this->db->select("*")->from($this->profile_enquiries_model->table_name());//IMPORTANT: without get() METHOD
		
		$this->load->library("Datatable");
		$this->datatable->setControllerKey('page_index');
		$this->datatable->setTitle("Enquiries");

		$this->datatable->setColumns(array("name"=>"profile_name", "column"=>"Profile Name"));
		...
		$this->datatable->setColumns(array("name"=>"message", "column"=>"Message"));
		...
		$this->datatable->buttons = array(array("label"=>"Export To Excel", 'link'=>admin_url("profile_list/enquiries/export")));
	...
	public function export(){
		...
		$institutes_data = $this->profile_enquiries_model->fetch_data();
		...
		$file_name = $this->profile_enquiries_model->table_name()."_".date("Y_m_d").".csv";
	...
	function remove($id=NULL){
		if(!is_null($id)){
			$this->profile_enquiries_model->remove($id);
			redirect($this->admin_url("profile_list/enquiries"));
		}else{
			redirect($this->admin_url("profile_list/enquiries"));
		}
	}
