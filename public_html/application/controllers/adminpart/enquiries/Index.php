...
class Index extends Admin_Controller {
	...
	public function __construct(){
		...
		$this->breadcrumb(array("enquiries"=>"Enquiries"));
	...
	public function index(){
		...
		$query = $this->db->select("*")->order_by('id', 'desc')->from($this->enquiries_model->table_name());//IMPORTANT: without get() METHOD
		...
		$this->datatable->setColumns(array("name"=>"email", "column"=>"Email"));
		$this->datatable->setColumns(array("name"=>"mobile", "column"=>"Mobile No."));
		...
		$this->datatable->buttons = array(array("label"=>"Export To Excel", 'link'=>admin_url("enquiries/export")));
	...
	public function export(){
		...
		$institutes_data = $this->enquiries_model->fetch_data();
		...
		$file_name = $this->enquiries_model->table_name()."_".date("Y_m_d").".csv";
	...
	function remove($id=NULL){
		if(!is_null($id)){
			$this->enquiries_model->remove($id);
			redirect($this->admin_url("enquiries"));
		}else{
			redirect($this->admin_url("enquiries"));
		}
	}
